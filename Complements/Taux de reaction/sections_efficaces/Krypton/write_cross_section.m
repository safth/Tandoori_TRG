...
for j=2:5 %écrit les 2P1 à 2P10
    fileID = fopen( sprintf('gs-1s%01d.txt',j),'at'); %ouvre ou crée un fichier en mode txt avec comme permission décrire un a la suite de lautre
    formatSpec = '%s \r\n'; %string (/r/n=saut de page sur windows)
    fclose(fileID);
end
...
for i=3:2:5 %écrit 1s3 et 1s5
    for j=2:5 %écrit les 2P1 à 2P10
        ...
        elseif j==i
            %ground state
            fileID = fopen( sprintf('1s%01d-gd.txt',i),'at'); %ouvre ou crée un fichier en mode txt avec comme permission décrire un a la suite de lautre
            formatSpec = '%s \r\n'; %string (/r/n=saut de page sur windows)
            fclose(fileID);
            %ionisation
